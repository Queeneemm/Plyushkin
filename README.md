...
- Отправка краткой сводки в разрешённые чаты и топики (если в чате включены темы).
...
- `/pool_aliases <id|id1,id2,...> <alias1,alias2>`
...
- В чатах с темами бот также запоминает `thread_id` (топик), если в него пишут.
- Отправка через меню: `Отправить сводку` → выбрать сессию → чат → (при наличии) топик.
- Ручная отправка: `/send_summary <session_id> <chat_id> [thread_id]`.
